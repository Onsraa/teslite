...
fn setup_ui(
    ...
    let bars_container = commands.spawn((Node {
    ...
    let accel_bar_container = commands.spawn( (Node {
        width: Val::Px(30.0),
        height: Val::Px(100.0),
    ...
    let accel_bar = commands.spawn((Node {
    ...
    let brake_bar_container = commands.spawn( (Node {
        width: Val::Px(30.0),
        height: Val::Px(100.0),
        border: UiRect::all(Val::Px(2.0)),
        ..default()
   ...
   BackgroundColor(Color::BLACK)
    ...
    let brake_bar = commands.spawn((Node {
        width: Val::Percent(100.0),
        height: Val::Percent(0.0),
        ..default()
    },
    ...
    commands.entity(bars_container).add_child(accel_bar_container);
    commands.entity(bars_container).add_child(brake_bar_container);

    let angle_text = commands.spawn((
        Text::new("Angle : 0°/0°"),
        TextFont {
            font: font.clone(),
            font_size: 16.0,
